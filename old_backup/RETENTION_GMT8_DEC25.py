    ...
    def allmemberReport(cls, team, bo_link, bo_name, currency, gmt_time, collection, g_sheet_ID, g_sheet_tab):
        ...
        while True:
            ...
            payload = {
                "paginate": paginate,
                "page": page,
                "from_date": f"2025-12-01T00:00:00+08:00",
                "to_date": f"2025-12-31T23:59:59+08:00",
                "currency": [currency],
                "merchant_id": 1,
                "admin_id": 295,
                "aid": 295
    ...
    def ssbo_allmemberReport(cls, merchant, currency, file_name, g_sheet_tab, g_sheet_ID):
        with sync_playwright() as p:  
            ...
            try:
                ...
                page.locator("//button[normalize-space()='Last Month']").click()
                # Delay 1 second
                page.wait_for_timeout(1000)
                # Button Click "Search"
...
while True:
    try:
        ...
        # safe_call(Fetch.allmemberReport, "IBS J8M", "jw8bo.com", "jw8", "MYR", "+08:00", "J8M_AMR_DEC25", "1FoD7rHmGBWkTx0QIGT_yw06-fG-mk7QyzJ84Ep1txVU", "TM - All Member Report")
        # print("======================================================\n")

        # print(">>== IBS J8M MY (PEI) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_AMR, "J8M_AMR_DEC25", "1I54nHH_M-3UXs4DqQGaqBhDewvKksqIWxyorbaMRaJU", "TM - All Member Report")
        # print("======================================================\n")

        # print(">>== IBS J8M MY (XY) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_AMR,"J8M_AMR_DEC25", "1vOikI71wExfzh5luDpX8KQKoINnsKVKYxBMqi-Js0a8", "TM - All Member Report")
        # print("======================================================\n")

        # print(">>== IBS J8S SG (CINDY) ==<<")
        # safe_call(Fetch.allmemberReport, "IBS J8S", "jw8bo.com", "jw8", "SGD", "+08:00", "J8S_AMR_DEC25", "1nCKZWdO2qaNy-4-9Zl86wGkZtd3C7x_HjsrjYF81YKo", "TM - All Member Report")
        # print("======================================================\n")
        ...
        # safe_call(Fetch.allmemberReport, "IBS A8M", "aw8bo.com", "aw8", "MYR", "+08:00", "A8M_AMR_DEC25", "1xy2C52zKX0o6Odcc3TysmPHXuXXhdYIJJyi1NoRrDBw", "TM - All Member Report (IBS)")
        # print("======================================================\n")
        
        # print(">>== IBS A8M MY (ANGIE 2) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_AMR, "A8M_AMR_DEC25", "1OtitCR8PXD9WXrOoXrluUdfMPZvzzzN__u4sulB9bio", "TM - All Member Report (IBS)")
        # print("======================================================\n")

        # print(">>== IBS A8M MY (AVA) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_AMR, "A8M_AMR_DEC25", "1bzFhQ6ji5Ch2sk-V1Cc3y2PNWk4CrSBllXhTN2deZX4", "TM - All Member Report (IBS)")
        # print("======================================================\n")
        ...
        # safe_call(Fetch.allmemberReport, "IBS A8S", "aw8bo.com", "aw8", "SGD", "+08:00", "A8S_AMR_DEC25", "1V_qWbLfSJloA6KtEW7QdXz9qkox53WYRP46yRePgD90", "TM - All Member Report (IBS)")
        # print("======================================================\n")

        # print(">>== IBS A8S SG (CINDY) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_AMR, "A8S_AMR_DEC25", "1JmVXGT67naNtM_9GiqPgDKqZXYdarKD-4nfy4zKTeMU", "TM - All Member Report (IBS)")
        # print("======================================================\n")
        ...
        # safe_call(Fetch.ssbo_allmemberReport, "Acewin8", "Malaysia", "SSBO_A8M_AMR_DEC25", "TM - All Member Report (SS)", "1xy2C52zKX0o6Odcc3TysmPHXuXXhdYIJJyi1NoRrDBw")

        # print(">>== SSBO A8M MY (ANGIE 2) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_SSBO_AMR, "SSBO_A8M_AMR_DEC25", "TM - All Member Report (SS)", "1OtitCR8PXD9WXrOoXrluUdfMPZvzzzN__u4sulB9bio")

        # print(">>== SSBO A8M MY (AVA) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_SSBO_AMR, "SSBO_A8M_AMR_DEC25", "TM - All Member Report (SS)", "1bzFhQ6ji5Ch2sk-V1Cc3y2PNWk4CrSBllXhTN2deZX4")

        # print(">>== SSBO A8S SG (AVA) ==<<")
        # safe_call(Fetch.ssbo_allmemberReport, "Acewin8", "Singapore", "SSBO_A8S_AMR_DEC25", "TM - All Member Report (SS)", "1V_qWbLfSJloA6KtEW7QdXz9qkox53WYRP46yRePgD90")

        # print(">>== SSBO A8S SG (CINDY) ==<<")
        # safe_call(mongodb_2_gs.upload_to_google_sheet_SSBO_AMR, "SSBO_A8S_AMR_DEC25", "TM - All Member Report (SS)", "1JmVXGT67naNtM_9GiqPgDKqZXYdarKD-4nfy4zKTeMU")

        print(">>== SSBO 9T (KOI) ==<<")
        safe_call(Fetch.ssbo_allmemberReport, "Ivip9", "Thailand", "SSBO_9T_AMR_DEC25", "TM - All Member Report", "1BNdnmoZT5yJL6F2ITUWGgCq9Em93VKH60wBFd_2UHZA")
        
        print(">>== SSBO A8T (VIEW) ==<<")
        safe_call(Fetch.ssbo_allmemberReport, "Acewin8", "Thailand", "SSBO_A8T_AMR_DEC25", "TM - All Member Report", "1ImzzSyHZOYkdp-v2RuNk6mzsLU77Fr3keG_FBsCvVpM")
