...
logger = logging.getLogger("superswan")
...
class mongodb_2_gs:
    ...
    @staticmethod
    def _find_first_empty_row(sheet, spreadsheet_id, gs_tab, start_column, end_column, start_row=3):
        """
        Return the first empty row (starting from `start_row`) within the target range.
        Uses values().get + values().update, not append.
        """
        range_name = f"{gs_tab}!{start_column}{start_row}:{end_column}"
    ...
    def upload_to_google_sheet_ssbo_DL_PID(cls, collection, gs_id, gs_tab, start_column, end_column, rows=None):
        ...
        clear_range = f"{gs_tab}!{start_column}3:{end_column}"
        sheet.values().clear(
            spreadsheetId=SPREADSHEET_ID,
            range=clear_range
        ).execute()
        ...
        target_range = f"{gs_tab}!{start_column}{first_empty_row}:{end_column}{end_row}"
    ...
    # Update Data to Google Sheet from MongoDB
    @classmethod
    def upload_to_google_sheet_DL_PID(cls, collection, gs_id, gs_tab, start_column, end_column, rows=None, extra_mongo_collections=None):
        ...
        clear_before_write = (start_column, end_column) in {("A", "C"), ("E", "G")}
        if extra_mongo_collections:
    ...
    def upload_to_google_sheet_DL_USERNAME(cls, collection, gs_id, gs_tab, start_column, end_column, rows=None):
        ...
        cls._sort_rows_by_datetime(rows, "completed_at")
        rows = sanitize_rows(rows)
...
class Fetch(Automation, BO_Account, mongodb_2_gs):
    ...
    def notify_listener(collection, gs_id, gs_tab, start_column, end_column, mode):
        ...
        print("⚠ Listener unreachable → continuing without notifying listener")
...
while True:
    try:
        ...
        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN}[EMILLIA] {Fore.YELLOW} DEPOSIT LIST Data...{Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.02)
        # SSBO A8M MY
        Fetch.ssbo_deposit_list_PID("aw8", ["MYR"], "SSBO_A8M_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "I", "K", upload_to_sheet=False)
        # SSBO A8S SG
        Fetch.ssbo_deposit_list_PID("aw8", ["SGD"], "SSBO_A8S_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "M", "O", upload_to_sheet=False)
        # # IBS J8M MY
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "MYR", "+08:00", "J8M_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "A", "C")
        # # IBS J8S SG 
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "SGD", "+08:00", "J8S_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "E", "G")
        # IBS A8M MY 
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "MYR", "+08:00", "A8M_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "I", "K", extra_mongo_collections=["SSBO_A8M_DL"])
        # IBS A8S SG  
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "SGD", "+08:00", "A8S_DL", "1EMg9G6-WgD-hbIbL6Z0YBPP_BqLP8MRpwHXLP_Ji9MA", "DEPOSIT LIST", "M", "O", extra_mongo_collections=["SSBO_A8S_DL"])
        
        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        # ============================================================== J8MS A8MS KAYREEN =============================================================================
        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN}[KAYREEN] {Fore.YELLOW} DEPOSIT LIST Data...{Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.02)
        ...
        # SSBO A8M MY
        Fetch.ssbo_deposit_list_PID("aw8", ["MYR"], "SSBO_A8M_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "I", "K", upload_to_sheet=False)
        # SSBO A8S SG
        Fetch.ssbo_deposit_list_PID("aw8", ["SGD"], "SSBO_A8S_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "M", "O", upload_to_sheet=False)
        # # IBS J8M MY
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "MYR", "+08:00", "J8M_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "A", "C")
        # # IBS J8S SG 
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "SGD", "+08:00", "J8S_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "E", "G")
        # IBS A8M MY 
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "MYR", "+08:00", "A8M_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "I", "K", extra_mongo_collections=["SSBO_A8M_DL"])
        # IBS A8S SG  
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "SGD", "+08:00", "A8S_DL", "1mg2YVYUWwut0XZ0MRg31X-1Wjp8FZRUbqrrimYKD_hE", "DEPOSIT LIST", "M", "O", extra_mongo_collections=["SSBO_A8S_DL"])

        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        # ============================================================== J8MS A8MS YVONNE =============================================================================
        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN}[YVONNE] {Fore.YELLOW} DEPOSIT LIST Data...{Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.02)
        ...
        # SSBO A8M MY
        Fetch.ssbo_deposit_list_PID("aw8", ["MYR"], "SSBO_A8M_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "EPOSIT LIST", "I", "K", upload_to_sheet=False)
        # SSBO A8S SG
        Fetch.ssbo_deposit_list_PID("aw8", ["SGD"], "SSBO_A8S_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "DEPOSIT LIST", "M", "O", upload_to_sheet=False)
        # # IBS J8M MY
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "MYR", "+08:00", "J8M_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "DEPOSIT LIST", "A", "C")
        # # IBS J8S SG 
        Fetch.deposit_list_PID("jw8bo.com", "jw8", "SGD", "+08:00", "J8S_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "DEPOSIT LIST", "E", "G")
        # IBS A8M MY 
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "MYR", "+08:00", "A8M_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "DEPOSIT LIST", "I", "K", extra_mongo_collections=["SSBO_A8M_DL"])
        # IBS A8S SG  
        Fetch.deposit_list_PID("aw8bo.com", "aw8", "SGD", "+08:00", "A8S_DL", "1n5gSpeFd24pGjdazgGvXay3rf57-SOdQbwUHCiD5mrE", "DEPOSIT LIST", "M", "O", extra_mongo_collections=["SSBO_A8S_DL"])
