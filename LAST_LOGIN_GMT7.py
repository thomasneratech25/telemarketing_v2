...
class mongodb_2_gs:
    ...
    def mongodbAPI_Last_Login(rows, collection):
        ...
        collection.create_index(
            [("member_id", 1), ("last_login_info_date", 1)],
            unique=True
        )
...
class Fetch(Automation, BO_Account, mongodb_2_gs):
    ...
    def last_Login(cls, bo_link, bo_name, team, currency, gmt_time, collection, gs_id, gs_tab):
        ...
        third_this_month = today.replace(day=3)
        third_this_month_str = third_this_month.strftime("%Y-%m-%d")

        # --- 3rd day of LAST month ---
        ...
        # Step 3: replace with day=3
        third_last_month = last_day_prev_month.replace(day=3)
        third_last_month_str = third_last_month.strftime("%Y-%m-%d")
        ...
        "vip_level": [],
        "last_login_from": third_last_month_str,
        "last_login_to": third_this_month_str,
        "merchant_id": 1,
        "admin_id": 205,
        "aid": 205
        }
...
while True:
    try:
        ...
        # S5T (MEMBER INFO)
        # safe_call(Fetch.last_Login, "s55bo.com", "s55", "S5T", "THB", "+07:00", "S55_S5T_LAST_LOGIN", "1E7YaLmoKGUDsb2pXylhQI9CFwBxI2L_wcwl_Fx5400A", "LAST LOGIN", description="S5T LAST LOGIN")
        safe_call(Fetch.ssbo_last_Login, "aw8", ["MYR"], "SSBO_A8M_LAST_LOGIN", "1E7YaLmoKGUDsb2pXylhQI9CFwBxI2L_wcwl_Fx5400A", "LAST LOGIN", description="SSBO A8M MY LAST LOGIN")
