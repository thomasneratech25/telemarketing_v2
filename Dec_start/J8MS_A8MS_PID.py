...
class Fetch(BO_Account, mongodb_2_gs):
    ...
    def deposit_list_PID(cls, bo_link, bo_name, currency, gmt_time, collection, gs_id, gs_tab, start_column, end_column, extra_mongo_collections=None):
        ...
        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN} {collection} {Fore.YELLOW} DEPOSIT LIST Data...{Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.01)
...
while True:
    try:
        ...
        # IBS D8M MY (SERINE)
        print("\n>>== IBS D8M MY (SERINE)==<<")
        safe_call(Fetch.deposit_list_PID, "dis88bo.com", "dis88", "MYR", "+08:00", "D8M_DL", "11jRZoxmKiRQmLFOTkksMrT25hKdtXlSlbMYU8g6M-L0", "DEPOSIT LIST", "A", "C")
        
        # IBS D8M MY (RY)
        print("\n>>== IBS D8M MY (RY)==<<")
        safe_call(mongodb_2_gs.upload_to_google_sheet_DL_PID, "D8M_DL", "1lbzsoka2arD6LHDI3krA3DmwsDHcaB7uaBlirfUEugY", "DEPOSIT LIST", "A", "C")
