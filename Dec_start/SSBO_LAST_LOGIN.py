...
class GoogleSheetHelper:
    ...
    # =-=-=--=-=-=-=-=-= SSBO =-=-=-=-=-=-=-=-=-=-=-=-=
    @classmethod
    def upload_to_google_sheet_ssbo_Last_Login(cls, gs_id, gs_tab, rows):
        ...
        rows = list(rows or [])
        cls._sort_rows_by_datetime(rows, "lastLoginDate")

        # Convert API dict rows to Google Sheet rows
        def sanitize_rows(raw_rows):
            sanitized = []
            for r in raw_rows:
                if isinstance(r, dict):
                    lastLoginDate = r.get("lastLoginDate") or ""
                    if lastLoginDate:
                        try:
                            ...
                            lastLoginDate = dt.strftime("%Y-%m-%d %H:%M:%S")
                        except Exception:
...
# Fetch Data
class Fetch(Automation, BO_Account, GoogleSheetHelper):
    ...
    @classmethod
    def ssbo_last_Login(cls, merchants, currency, gs_id, gs_tab):
        ...
        while True:
            ...
            if response.status_code in (401, 403):
                ...
                cls.ssbo_last_Login(merchants, currency, gs_id, gs_tab)
                return
            ...
            # Keep only rows with expected SSBO fields
            valid_rows = [
                r for r in rows
        ...
        if all_rows:
            cls.upload_to_google_sheet_ssbo_Last_Login(gs_id, gs_tab, all_rows)
        else:
            print("No SSBO rows collected to upload to Google Sheet.")
...
while True:
    try:
        ...
        # SSBO
        # safe_call(Fetch.ssbo_last_Login, "aw8", ["MYR"], "1vAmjUff6yxKyBgZGqu0JZzAihOUOIMyKpx4Gpxmtf5U", "LAST LOGIN", description="SSBO A8M MY LAST LOGIN")
