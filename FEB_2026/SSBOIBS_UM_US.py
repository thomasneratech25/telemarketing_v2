...
class mongodb_2_gs:
    ...
    def upload_to_google_sheet_DL_PID(cls, collection, gs_id, gs_tab, start_column, end_column, rows=None, extra_mongo_collections=None, overwrite=False, upload_to_sheet=True):
        ...
        base_rows = cls._normalize_pid_rows(rows)
        extra_rows = []
        if extra_mongo_collections:
            extra_rows = cls._fetch_extra_pid_rows(extra_mongo_collections)
            print(
                f"DL PID merge counts -> base={len(base_rows)} "
                f"extra={len(extra_rows)} total={len(base_rows) + len(extra_rows)}"
            )
...
class Fetch(BO_Account, mongodb_2_gs):
    ...
    def deposit_list_PID(cls, bo_link, bo_name, currency, gmt_time, collection, gs_id, gs_tab, start_column, end_column, extra_mongo_collections=None):
        ...
        # Upload Data to Google Sheet by reading from MongoDB
        cls.upload_to_google_sheet_DL_PID(
            collection,
            gs_id,
            gs_tab,
            start_column,
            end_column,
            extra_mongo_collections=extra_mongo_collections
        )
...
while True:
    try:
        ...
        # IBS UM MY
        safe_call(Fetch.ftd_stdReport, "29018465.asia", "uea8", "MYR", "+08:00", "UM_FTD_STD", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "UM", upload_to_sheet=False, description="IBS UM MY FTD/STD")
        # SSBO UM MY
        safe_call(Fetch.ssbo_ftd_stdReport, "uea", "MYR", "SSBO_UM_FTD_STD", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "UM", extra_mongo_collections=["UM_FTD_STD"], description="SSBO UM MY FTD/STD")

        # SSBO US SG
        safe_call(Fetch.ssbo_ftd_stdReport, "uea", "SGD", "SSBO_US_FTD_STD", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "US", description="SSBO US SG FTD/STD")
        ...
        # SSBO UM MY
        safe_call(Fetch.ssbo_deposit_list_PID, "uea", ["MYR"], "SSBO_UM_DL", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "DEPOSIT LIST", "A", "C", upload_to_sheet=False, description="SSBO UM MY DL PID")
        # IBS UM MY 
        safe_call(Fetch.deposit_list_PID, "29018465.asia", "uea8", "MYR", "+08:00", "UM_DL", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "DEPOSIT LIST", "A", "C", extra_mongo_collections=["SSBO_UM_DL"], description="IBS UM MY DL PID")
        
        # SSBO US SG
        safe_call(Fetch.ssbo_deposit_list_PID, "uea", ["SGD"], "SSBO_US_DL", "1YXrPU7c5_-YmGRfZRVLRLX-FKyHBr2oNAt1xTtfGgfA", "DEPOSIT LIST", "E", "G", description="SSBO US SG DL PID")
        ...
        # IBS UM MY
        safe_call(Fetch.member_info, "29018465.asia", "uea8", "MYR", "+08:00", "UM_MI", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "UM", upload_to_sheet=False, description="IBS UM MY MEMBER INFO")
        # SSBO UM MY
        safe_call(Fetch.ssbo_member_info, "uea", "MYR", "SSBO_UM_MI", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "UM", extra_mongo_collections=["UM_MI"], description="SSBO UM MY MEMBER INFO")

        # SSBO US SG
        safe_call(Fetch.ssbo_member_info, "uea", "SGD", "SSBO_US_MI", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "US", description="SSBO US SG MEMBER INFO")
        ...
        # SSBO UM MY
        safe_call(Fetch.ssbo_deposit_list_PID, "uea", ["MYR"], "SSBO_UM_DL", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "DEPOSIT LIST", "A", "C", upload_to_sheet=False, description="SSBO UM MY DL PID")
        # IBS UM MY 
        safe_call(Fetch.deposit_list_PID, "29018465.asia", "uea8", "MYR", "+08:00", "UM_DL", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "DEPOSIT LIST", "A", "C", extra_mongo_collections=["SSBO_UM_DL"], description="IBS UM MY DL PID")
        
        # SSBO US SG
        safe_call(Fetch.ssbo_deposit_list_PID, "uea", ["SGD"], "SSBO_US_DL", "1wv4Ld48BwJFZFyo9CFRJHk69GSR5n3rM8N08s3Tmn2Q", "DEPOSIT LIST", "E", "G", description="SSBO US SG DL PID")
