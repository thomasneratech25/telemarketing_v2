...
class mongodb_2_gs:
    ...
    @staticmethod
    def _normalize_username_rows(rows):
    ...
    @classmethod
    def _fetch_extra_username_rows(cls, collection_names):
        """Load and normalize USERNAME docs from extra Mongo collections."""
        ...
        for col_name in collection_names:
            ...
            combined.extend(cls._normalize_username_rows(docs))
        return combined
    ...
    # ====================================================================================
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-= SSBO DEPOSIT LIST (USERNAME) =-=-=-=-=-=-=-=-=-=-=-=-
    # ====================================================================================

    # MongoDB Database
    def mongodbAPI_ssbo_DL_USERNAME(rows, collection):
    ...
    @classmethod
    def upload_to_google_sheet_ssbo_DL_USERNAME(cls, collection, gs_id, gs_tab, start_column, end_column, rows=None):
        ...
        def sanitize_rows(raw_rows):
            ...
            for r in raw_rows:
                if isinstance(r, dict):
                    username = str(r.get("memberLogin", ""))
                    ...
                    username = str(r[0]) if len(r) > 0 else ""
                    if username and not username.startswith("'"):
                        username = f"'{username}"
                    sanitized.append([
                        username,
...
class Fetch(BO_Account, mongodb_2_gs):
    ...
    def ssbo_member_info(cls, merchants, currency, collection, gs_id, gs_tab, extra_mongo_collections=None):
        ...
        # Print title
        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN} {collection} {Fore.YELLOW} ND ... {Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.01)
    ...
    @classmethod
    def ssbo_ftd_stdReport(cls, merchants, currency, collection, gs_id, gs_tab, extra_mongo_collections=None):
        ...
        while page < max_pages:
            ...
            if response.status_code in (401, 403):
                ...
                # Retrying requests...
                print("⚠️ Cookies + bearer token refreshed ... Retrying request...\n")
                return cls.ssbo_ftd_stdReport(merchants, currency, collection, gs_id, gs_tab, extra_mongo_collections=extra_mongo_collections)
        ...
        # upload to google sheet
        cls.upload_to_google_sheet_ssbo_FTD_STD(collection, gs_id, gs_tab, extra_mongo_collections=extra_mongo_collections)
    ...
    # SSBO Deposit List (USERNAME)
    @classmethod
    def ssbo_deposit_list_USERNAME(cls, merchants, currency, collection, gs_id, gs_tab, start_column, end_column, upload_to_sheet=True):
        ...
        while True:
            ...
            if response.status_code in (401, 403):
                ...
                print("⚠️ Cookies + bearer token refreshed ... Retrying request...\n")
                cls.ssbo_deposit_list_USERNAME(merchants, currency, collection, gs_id, gs_tab, start_column, end_column)
                return
            ...
            # Insert into MongoDB
            if rows:
                cls.mongodbAPI_ssbo_DL_USERNAME(rows, collection)
            else:
        ...
        if upload_to_sheet:
            cls.upload_to_google_sheet_ssbo_DL_USERNAME(collection, gs_id, gs_tab, start_column, end_column)
...
while True:
    try:
        ...
        # SSBO 9T ARRIN
        safe_call(Fetch.ssbo_deposit_list_USERNAME, "ip9", ["THB"], "SSBO_9T_DL", "1Xr5ZZwp-ESPvoMeFEQ2Zq1SaIum5vFbskKnmyh71AlM", "DEPOSIT LIST", "A", "C", description="SSBO 9T deposit list")
        
        # SSBO 9T SUPATTRA
        safe_call(mongodb_2_gs.upload_to_google_sheet_ssbo_DL_USERNAME, "SSBO_9T_DL", "1aOrdFTvzQmctMuXhjiZ5BIwycugD2eBkxGiai2U73rg", "DEPOSIT LIST", "A", "C", description="SSBO 9T deposit list")
        ...
        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        # ============================================================== SSBO A8T DEPOSIT LIST USERNAME  ================================================
        # =-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        # # SSBO A8V 
        # safe_call(Fetch.ssbo_deposit_list_USERNAME, "aw8", ["VND"], "SSBO_A8V_DL", "", "DEPOSIT LIST", "A", "C", description="SSBO A8V deposit list")
