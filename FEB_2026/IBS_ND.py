...
class mongodb_2_gs:
    ...
    # MongoDB Database (ND)
    def mongodbAPI_ND(rows, collection):
    ...
    # Update Data to Google Sheet from MongoDB (ND)
    @classmethod
    def upload_to_google_sheet_ND(cls, collection, gs_id, gs_tab, rows=None):
...
class Fetch(BO_Account, mongodb_2_gs):
    ...
    # Member Info
    @classmethod
    def ND(cls, bo_link, bo_name, team, currency, gmt_time, collection, gs_id, gs_tab):
        
        session = create_session()

        # Print Team Name Messages
        msg = f"\n{Style.BRIGHT}{Fore.YELLOW}Getting {Fore.GREEN} {team} {Fore.YELLOW} ND Data...{Style.RESET_ALL}\n"
        for ch in msg:
            sys.stdout.write(ch)
            sys.stdout.flush()
            time.sleep(0.01)
        ...
        "register_from": first_day_last_month_str,
        "register_to": last_day_last_month_str,
        "merchant_id": 1,
        "admin_id": 581,
        "aid": 581
        }
        ...
        if response.json().get("statusCode") == 401:
            ...
            # Retry request...
            return cls.ND(bo_link, bo_name, team, currency, gmt_time, collection, gs_id, gs_tab)
        ...
        for page in range(1, 10000): 
            ...
            if "data" in data and len(data["data"]) > 0:
                cls.mongodbAPI_ND(data["data"], collection)
            else:
                print("No data returned from API.")

        # Upload Data to Google Sheet by reading from MongoDB
        mongodb_2_gs.upload_to_google_sheet_ND(collection, gs_id, gs_tab)
...
# safe_call(Fetch.ND, "BO Link", "merchant code", "team name", "currency", "gmt time", "database name", "google sheet link", "google sheet tab name", description="can write any")
...
while True:
    try:
        ...
        # NAREM
        safe_call(Fetch.ND, "gcwin99bo.com", "gc99", "GT", "THB", "+07:00", "GT_ND", "1X3_U_OaqDp7IYEVITUh-PaLJtq3VbV6MzI58wRzXcLs", "ND", description="GT ND")
        ...
        # ELSA
        safe_call(Fetch.ND, "f5x3n8v.com", "n855", "N855T", "THB", "+07:00", "N855T_ND", "1PhrRokHUYNLKvI8QWINUD5DtbhtJNkkToDZB0k4frjQ", "ND", description="N855T ND")
        ...
        # SOFIA
        safe_call(Fetch.ND, "q2n5w3z.com", "n789", "N789T", "THB", "+07:00", "N789T_ND", "15jh_5Aulx-PFwA73UhUg32iLon_kiLWw3reik-0t0uU", "ND", description="N789T ND")
